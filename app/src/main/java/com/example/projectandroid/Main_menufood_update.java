...
public class Main_menufood_update extends AppCompatActivity {

BottomNavigationView bottomNavigationView;
    ListView list;
    Spinner spinner_danhmuc;
    FoodAdapter_update foodAdapter;
...
//        Intent intent = getIntent() ;
        ...
        query.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ArrayList<Food_update> arrayList = new ArrayList<>();
                    List<DocumentSnapshot> documents = task.getResult().getDocuments();
                    if(type.isEmpty())
                    {
                        for (DocumentSnapshot document : documents) {
                            arrayList.add(new Food_update(document.get("img").toString(),document.get("name").toString(),document.get("mota").toString(),document.get("price").toString(),document.getId()));
                        }
                    }
                    else{
                        for (DocumentSnapshot document : documents) {
    ...
    {
        Intent intent = new Intent(Main_menufood_update.this,Main_menutopping_update.class);
